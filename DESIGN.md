...
- **content**: app-shell の中身を 1 枚の大きな DQ ウィンドウで包む。内側に置く個別ウィンドウは `.dq-window` でさらに二重枠にする (DQ で「持ち物」の中に「やくそう ×3」が表示される入れ子構造)。
  ...
  - *モバイルは全ルート全面ダーク*: 上記に加え、`body` を不透明ダーク (`#0e1318`) にして青空/草原を一切出さず、`.content` の余白・外枠と footer-nav の中央寄せピルを撤去して header + 本文 + footer をダークで画面いっぱいに敷き詰める (情報密度優先のオーナー判断)。**workspace (`/`) だけでなく footer から開く通知/検索などの単独ルートも同じ全幅・枠なし・全幅 footer に統一** (旧: workspace だけ全幅で他は白枠+ピル footer の二重構造だった)。カラム本体は左右 0.5em の余白を持ち、フィード投稿はその余白に揃えて全幅・下辺ヘアライン (白 0.55) で区切る。footer は全幅フラッシュ + `env(safe-area-inset-bottom)` でホームインジケータを回避。**青空+草原に手帳が浮かぶ世界観は PC のみ** (モバイルで背景を出すよう差し戻さないこと)。
