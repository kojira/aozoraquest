...
export const BATTLE_TUNING = {
  ...
  xpWin: 30,
  /**
   * 敗北 XP。**0 = 負けても経験値は入らない** (#621)。
   *
   * 以前は 5 を配っていたが、tier1 のスライム 3 種は勝利 XP が 3〜5 しかなく
   * **負けたほうが多い (または同じ)** という逆転が実在した (実測)。序盤ほど遭遇する
   * 敵なので踏みやすい。強い敵ほど勝利 XP が上がる設計と、固定額の敗北 XP は
   * 原理的に噛み合わない (敵を弱く足すたびに再発する) ため、配るのをやめる。
   * 負けたときは素材ロスとパワー消費だけが残る。
   */
  xpLose: 0,
