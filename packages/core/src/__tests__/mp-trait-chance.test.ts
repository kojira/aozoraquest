...
/**
 * **パラディンの MP 回復を確率にする** (#564。オーナー決定 2026-07-27)。
 *
 * `祈りの加護` は毎ターン確実に MP が戻るため、回復とくぎ (聖光の癒し) と噛み合って
 * **資源が尽きず tier1 を無限に周回できていた** — 素手・回復なしの連戦で 10 試行すべて
 * 300 戦連勝し、MP は 18/18 のまま張り付いていた。「祈りが通じるかはそのとき次第」
 * = 確率発動にする。
 *
 * `chance` を持たないジョブは**乱数を 1 つも引かない**こと。引くと乱数ストリームが
 * ずれて他ジョブの戦闘結果まで変わる (テストと world-data の決定論が壊れる)。
 */
