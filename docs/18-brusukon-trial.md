...
- プレイヤーの戦闘値は**プロフィールの個人 5 パラメータ (rpgStats、合計 100)** を基底に、
  jobLevel/playerLevel 補正を乗じて導出 (`playerCombatant` の baseStats 引数。
  オーナー指摘 2026-07-17)。未診断などで rpgStats が無い場合のみジョブ基準値に
  フォールバック。
