...
本体 `aozoraquest.app` とは別の **Cloudflare Workers Builds プロジェクト** として配信する。理由:
...
本体も管理 SPA も、どちらも **Cloudflare Workers Builds による静的配信のみ**。Cloudflare Worker は使わない。
...
- LLM は管理画面に不要 (`wasm-unsafe-eval` を含めない)
...
LLM 呼び出し (Chrome の Gemini Nano) 時、PDS から取得した system プロンプトを渡す:

```typescript
const prompt = configs.prompts.spiritChat?.body ?? DEFAULT_SPIRIT_PROMPT;
const result = await generateWithLocalLLM({ systemPrompt: prompt, history: messages });
```

`DEFAULT_SPIRIT_PROMPT` はアプリコード内に保持 (PDS 障害時・取得失敗時のフォールバック)。クラウド LLM API は採用しない方針なので (10-roadmap.md §決定ログ 2026-05-27)、プロバイダー差異への配慮は不要。
...
│ [→ Actions を開く (admin)]                   │
│                                              │
│ 最近のデプロイ履歴: (Cloudflare Workers Builds)       │
│ [→ Pages ダッシュボードを開く]               │
└─────────────────────────────────────────────┘
```
...
│ バージョン切り替え                           │
│                                              │
│ ロールバックは Cloudflare Workers Builds から         │
│ 実行してください。                            │
│                                              │
│ [→ Pages: web プロジェクト]                  │
│ [→ Pages: admin プロジェクト]                │
...
Cloudflare Workers Builds ダッシュボードの「Deployments」からワンクリックで過去のデプロイを再公開できる。

### (i) 利用メトリクス

**採用しない**。解析サービス・トラッキング・Cookie を導入しない方針 (README の差別化主張と整合)。アクセス概況は Cloudflare ダッシュボードの基本リクエスト統計を運用者が手元で見る範囲にとどめる。
...
- **06-spirit.md**: システムプロンプトの保存場所が「リポジトリ直書き」から「主管理者 PDS (管理画面経由で編集可)」に変わる旨を §自由対話 セクションに注記。`DEFAULT_SPIRIT_PROMPT` フォールバックはコードに残す
...
- **ユーザー行動の個別トラッキング**: 独自集計は行わない。解析サービスも導入しない
