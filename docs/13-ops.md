...
- AT Protocol OAuth の access token は DPoP バインドされ、ブラウザのみで管理
- 投稿内容を外部 LLM API に送信しないため、API キー類は保管しない (10-roadmap.md §決定ログ)
...
- 要明記: LLM 推論が端末内で完結すること、管理者 PDS への公開コンフィグ読み取り、共鳴 TL のためのディレクトリ利用
