...
実際の配信ヘッダは `apps/web/public/_headers` に集約 (Cloudflare Workers Builds が読む)。本番動作を壊さない**最小セットを enforce**、より厳しい**完全版を Report-Only**で観察する二段構えにしている。
...
クリックジャッキング、`<base>` 経由のリソース乗っ取り、`<object>`/`<embed>` 経由のプラグイン読み込みを禁止。これだけで実害ある攻撃面の大半は塞げる。
