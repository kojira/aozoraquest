...
- **あおぞらパワー** (#551 段階 1) — 残高の正は `GameState.power` だけ。増減の経路:
  - 投稿 → `/api/xp/claim` が +1
  - 戦闘 → `battle-reward` が −1
  - しらべる → `/api/world/search` が −1 (以前は client 台帳だけで、いくらでも しらべられた)
...
- **装備の個体** (`craft` レコード) — 強化値の抽選はサーバーに移したが、レコード自体は
  ユーザー PDS にあり、本人が偽造できる。gear の解決もそこを読む (#551 段階 2)
- **素ステ** (`analysis.rpgStats`) — 一番深い。サーバーが投稿を読んで自分で算出するまで消えない (段階 3)
