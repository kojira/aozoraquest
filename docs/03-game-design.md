...
ATK / DEF / AGI / INT / LUK の順、合計 100。各ジョブの配分は手調整ではなく、
dominant/auxiliary の認知機能スタック (dom 1.0 / aux 0.5 / tertiary 0.25 /
inferior 0.125) を `COGNITIVE_TO_RPG` で合成・正規化した理論値に、レーダー軸
潰れ防止の floor (v = 6 + 0.7×理論値、affine なので形は不変) を噛ませて算出する。
生成・検証は `scripts/gen-job-stats.ts`。これにより 16 型 MBTI と配分が厳密に整合する。

| id | 既定名 | 生産系 | 別系統 | ATK | DEF | AGI | INT | LUK | 主機能 | 補助機能 |
|---|---|---|---|---|---|---|---|---|---|---|
| `sage` | 賢者 | 建築家 | 戦略家 | 22 | 15 | 9 | 40 | 14 | Ni | Te |
| `mage` | 魔法使い | 錬金術師 | 研究者 | 7 | 19 | 19 | 37 | 18 | Ti | Ne |
| `shogun` | 将軍 | 棟梁 | 起業家 | 39 | 10 | 13 | 28 | 10 | Te | Ni |
| `bard` | 吟遊詩人 | 発明家 | 即興師 | 7 | 14 | 27 | 21 | 31 | Ne | Ti |
| `seer` | 予言者 | 導師 | 語り部 | 8 | 16 | 10 | 43 | 23 | Ni | Fe |
| `poet` | 詩人 | 職人 | 彫刻家 | 11 | 32 | 15 | 8 | 34 | Fi | Ne |
| `paladin` | 聖騎士 | 教育者 | 案内人 | 9 | 19 | 13 | 25 | 34 | Fe | Ni |
| `explorer` | 冒険者 | 触媒 | 旅芸人 | 14 | 19 | 25 | 8 | 34 | Ne | Fi |
| `warrior` | 戦士 | 書記 | 鍛冶師 | 25 | 41 | 8 | 13 | 13 | Si | Te |
| `guardian` | 守護者 | 司書 | 家守 | 10 | 43 | 9 | 17 | 21 | Si | Fe |
| `fighter` | 匠 | 技師 | 達人 | 12 | 12 | 23 | 43 | 10 | Ti | Se |
| `artist` | 芸術家 | 工芸家 | 庭師 | 15 | 26 | 19 | 14 | 26 | Fi | Se |
| `captain` | 隊長 | 指揮者 | 管理官 | 38 | 23 | 11 | 15 | 13 | Te | Si |
| `miko` | 巫女 | 世話役 | 看護師 | 8 | 32 | 11 | 12 | 37 | Fe | Si |
| `ninja` | 忍者 | 職方 | 現場監督 | 17 | 11 | 34 | 25 | 13 | Se | Ti |
| `performer` | 遊び人 | 芸人 | 祭司 | 25 | 16 | 32 | 11 | 16 | Se | Fi |
...
### 転職してもレベルは職ごとに保持される (#531)

**転職で元の職のレベルは下がらない。過去に育てた職に戻れば、そのレベルの続きから再開できる。**

XP の出所ごとの扱い:

| 出所 | 転職したら | 保存場所 |
|---|---|---|
| 投稿・診断 | **職ごとに保持** (保管庫 `jobXpByArchetype` に退避 → 戻ったら復元) | `analysis` (ユーザー PDS) |
| 戦闘 | **職ごとに保持** (元から archetype キー) | `GameState.jobXp` (サーバー権威) |

| 依頼クエスト | **全職に等しく乗る** (完了レコードに「どの職で受けたか」が無いため分けられない) | 完了集合からの派生 |

※ 戦闘 XP が実際に LV に効くのは #529 以降。それ以前は `GameState.jobXp` は書かれるだけで
どの LV にも反映されていなかった。

付け替えは `switchJobXp()` (core/quest.ts) に集約する。転職経路は 3 つ (ユーザーの転職確定 /
管理ツールの職変更 / 再診断で職が変わる場合) あり、片方だけ直す事故を防ぐため 1 箇所にまとめる。

**現職ぶんは `jobLevel.xp` が正**で保管庫には入れない (二重計上の防止)。保管庫を持たない
既存ユーザーは「過去分ゼロ」として扱われ、今までどおり動く。

管理ツールの LV 直接指定 (`adminSetJob`) は**一時的な上書き**として扱い、転職先の過去分も
保管庫に残す。消してしまうと「戦士 Lv30 → admin で賢者 → admin で戦士 Lv5」で戦士の XP が
復旧不能に失われる。
