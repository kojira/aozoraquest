...
- LLM 推論はすべて端末内 (Chrome の Gemini Nano + ONNX/WASM の Ruri-v3 ベース埋め込みモデル)。クラウド LLM API は使わない
...
| `settings` | ユーザー設定 (表示名バリアント、各種トグル等) | 無期限 |
...
未対応ブラウザでは生成系機能を OFF にして案内する (gemini-nano-availability.ts)。
...
### パターン 3: 精霊との会話 (ローカル LLM)

```
User sends message to spirit
  ↓
Client → Browser LLM (Chrome の Gemini Nano)
  ↓
LLM stream response (端末内、外部送信なし)
  ↓
...
Gemini Nano 未対応ブラウザでは精霊会話 UI を OFF にして案内 (06-spirit.md §フォールバック)。
...
3. それも失敗したら診断機能を無効化し、対応ブラウザ (Chrome デスクトップ) で開き直すよう案内する
...
**保管しない**。クラウド LLM API への送信機能 (BYOK) を採用していないため、API キー保管に関する設計は不要 (10-roadmap.md §決定ログ参照)。一般的な防御 (XSS 対策、第三者 JavaScript の不使用、CSP の厳格化) は継続する。
