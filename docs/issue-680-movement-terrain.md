...
- 既に描画中なら残りの直交step列へ追加。現在位置を引継ぎ、方向転換で斜めに角を切らない。前stepから200ms以内を連続とし、170ms未満の速いrepeat/転向は受理間隔（下限16ms）から速度を求める。170..200msは170ms固定。長い間隔後は170msへ戻す。通信や論理移動頻度は不変。
...
## 独立レビュー対応
- P1: 速度補正が120ms未満に限られ、125ms等の連続入力では描画経路が増え続ける。修正前を回帰で確認してから、既存速度補正の閾値を標準周期170msへ合わせる。受理/通信/経路は不変。
- 受入: 125msの5秒長押しで残距離が2マス未満、離して標準の1歩+初回バッファ（186ms）以内に整列。境界120/169msも同じ回帰で検証し、既存170ms一定速度/転向/高速入力は維持。実World回帰も修正後に確認する。限定差分と結果を親へ渡し再判定後にのみmergeする。
- 親が限定差分とred→greenを読みP1解消と再判定（最新HEAD CI成功条件）。追加要請の上方向/NPC外周は実World回帰へ追加し成功。上端外のNPCが地面と同じscreen matrix変位で流れ込み、停止時0へ揃うフレームを記録。アプリsourceの追加修正なし。
