...
| `app.aozoraquest.questLog` | システム日次クエストの進捗と履歴 | タイムスタンプベース |
| `app.aozoraquest.companion` | 旅の仲間リスト | `self` |
| `app.aozoraquest.companionLog` | 精霊との会話履歴 | タイムスタンプベース |
| **`app.aozoraquest.userQuest`** | **依頼クエスト本体 (発注者の PDS、15-user-quest.md)** | tid |
| **`app.aozoraquest.questApplication`** | **依頼クエストへの応募 (応募者の PDS)** | tid |
| **`app.aozoraquest.questCompletion`** | **完了報告 / 承認 / やり直し依頼 (書き手の PDS)** | tid |

### admin 系 (主管理者の PDS、env 共有)

| NSID | 内容 | rkey |
|---|---|---|
| `app.aozoraquest.directory` | 共鳴 TL の opt-in DID リスト (手動運用) | `self` |
| `app.aozoraquest.config.flags` | 機能フラグ | `self` |
| `app.aozoraquest.config.maintenance` | メンテナンスバナー | `self` |
| `app.aozoraquest.config.bans` | BAN リスト | `self` |
| `app.aozoraquest.config.prompts` | プロンプトテンプレート | `self` |
| **`app.aozoraquest.questIndex`** | **依頼クエスト集約インデックス (Worker 自動運用、15-user-quest.md)** | env 別 (`self` / `dev` 等) |

`rkey = self` は「1 ユーザーにつき 1 レコード」のシングルトン扱い。

**`questLog` (システム日次) と `userQuest` (依頼クエスト) は別物**。前者はユーザーがその日にクリアしたシステム発行クエスト記録、後者はユーザー発行の依頼掲示板アイテム。混同回避のため、依頼クエスト系統はすべて `quest*` (Application / Completion / Index) で命名統一する。詳細は `15-user-quest.md` を参照。
...
精霊との会話履歴。ローカル LLM (Chrome の Gemini Nano) で自由対話を行ったユーザーのみ使用。rkey はタイムスタンプ。
...
interface AozoraDB {
  ...
  settings: {
    ...
    hideReposts?: boolean;
    // 注: BYOK 関連フィールドは保持しない (10-roadmap.md §決定ログ: クラウド LLM API は採用しない)
  };
