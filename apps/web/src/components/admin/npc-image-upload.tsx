...
export function NpcImageUpload({ npc, canUpload, onFile, onRemove }: {
  ...
  return <section aria-label="NPC画像アップロード" style={{ border: '1px solid var(--color-border)', padding: '0.7em', fontSize: '0.85em', minWidth: 0, overflowWrap: 'anywhere' }}>
    ...
    <p>PNG・WebP対応、透明背景OK。JPEG / GIF / APNG / アニメWebP / SVGは静止PNG・WebPへ変換してください。</p>
    <p>選択だけでは送信しません。「保存」でPDSへ送信し、誰でも見られるゲーム素材として公開します。画像は無変換で保存します。位置情報などの付加情報がある場合もそのまま公開されます。保存失敗時も送信済み画像がPDSに残る場合があり、自動削除はしません。</p>
    ...
    {npc.spriteImage && <p>{npc.spriteImage.width}×{npc.spriteImage.height}px · {npc.spriteImage.width === npc.spriteImage.height ? '静止' : '横2コマ'} <button type="button" onClick={() => onRemove('sprite')}>マップ画像を外す</button></p>}
    ...
    {npc.portraitImage && <p>{npc.portraitImage.width}×{npc.portraitImage.height}px <button type="button" onClick={() => onRemove('portrait')}>会話イラストを外す</button></p>}
