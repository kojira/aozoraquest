 ...
 *  Bluesky 世界観のコスト表現 (投稿削除・フォロワ喪失・いいね消費 等) を混ぜる。 */
...
/** LLM に与える Bluesky 世界観リファレンス。全 rarity 共通でプロンプトに入れる。 */
const BLUSKY_REFERENCE = [
  '世界観: 旅人たちは Bluesky (青空の広場) で繋がっている。',
  'コストで使える要素: 投稿を削除する / フォロワを N 人失う / いいねを N 個消費する /',
  '  リポストを 1 回する / 引用返信を 1 つする / 下書きを 1 本捨てる / タイムラインを一時封じる /',
  '  仲間を生贄に捧げる / カードをタップする / あおぞらパワー N を支払う。',
  '説明で使えるネタ: 相手の投稿を一時的に隠す / 場のフォロワが騒ぐ / ハッシュタグが広がる /',
  '  引用の連鎖が止まる / 誰かの下書きが完成する / 眠れる通知が一斉に鳴る / など。',
  '能力名の参考: 驚天動地 / 世界征服 / 先制攻撃 / 沼わたり / 言霊 / 炎上鎮火 / 引用封じ /',
  '  リポ螺旋 / 沈黙の波紋 / 双頭の筆 / 薄明の呼応 / 等、RPG 調と Bluesky 調どちらもあり。',
].join('\n');
...
function buildPrompt(result: DiagnosisResult, rarity: Rarity): { system: string; user: string } {
  ...
  const system = [
    ...
    '能力名: <2〜5字、RPG 調 or Bluesky 調のキーワード>',
    'コスト: <日本語、Bluesky 要素を混ぜてよい。なし でも可>',
    '説明: <20〜40字、Bluesky 世界観のネタを使ってよい>',
