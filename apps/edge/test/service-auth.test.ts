...
import { verifyServiceAuth, decodeMultikey, ServiceAuthError, type Curve } from '../src/service-auth';
...
function makeJwt(curve: Curve, priv: Uint8Array, payload: Record<string, unknown>): string {
  ...
  const sig = CURVES[curve].sign(sha256(signingInput), priv).toCompactRawBytes();
