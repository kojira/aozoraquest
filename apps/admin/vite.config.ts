...
/**
 * Admin 用 client-metadata.json を build 時に生成する。
 * VITE_APP_URL に応じて client_id / redirect_uris を切り替える。
 * 未指定時は build を失敗させる (silent に誤デプロイを防ぐ)。
 */
function clientMetadataPlugin(): Plugin {
  return {
    name: 'aozoraquest-admin-client-metadata',
    apply: 'build',
    generateBundle() {
      const raw = process.env.VITE_APP_URL;
      if (!raw) {
        throw new Error(
          'VITE_APP_URL is required at build time (e.g. https://admin.aozoraquest.app). ' +
      ...
      const appName = process.env.VITE_APP_NAME || 'Aozora Quest Admin';
      const metadata = {
