...
### スピリットとの会話
精霊キャラクターがローカル LLM で語りかけてくる。会話のたびに LLM へあなたのジョブとレベルが渡されるので、診断結果を踏まえた応答になる。

### フォロー相性ランキング
フォロー相手との「気質の共鳴度」を一覧表示。Bluesky の通常ユーザーだけでなく、未診断のアカウントも裏で診断して対象にする。

### Bluesky クライアントとしても完成度高め
タイムライン、投稿・引用リポスト・リプライ、スレッドのインライン展開、通知 (グルーピング表示)、検索 (ユーザー / 投稿 / ハッシュタグ)、プロフィール表示、画像 lightbox、外部リンク OG カード、設定など。診断・カード・スピリット機能を切っても普通の Bluesky クライアントとして使える。
