...
### 既定は 1 観点・少数精鋭 (2026-07-26 改定)

PR 作成後、CI 緑になった時点で **subagent 1 体 (実装観点) にレビューさせる**。

- 指摘は **★★★ / ★★ の 2 段階だけ。★ (改善余地) は出させない**
- **合計 5 件まで**に絞らせる
- **「どの入力 / 操作 → どの誤った結果 → 再現条件」を書けない指摘は出させない**。
  「読みにくい」「命名が」「〜した方がよい」は対象外
- 推測でなく、コードを読むか一時スクリプトで**実測して**裏付けさせる
- subagent は `claude.ai/code` の管理外で動く独立観点。**ultrareview のような有料ツールは使わない**

### 3 観点に上げる条件 (限定)

以下に**該当するときだけ** 3 並列 (設計 / 実装 / UX) に上げる:

- **dev → main の本番リリース PR**
- **権威データ (GameState) のスキーマ変更・移行**を含む
- **ユーザーのデータが消えうる**変更 (リセット / 上書き / マイグレーション)
- **パワー / XP / 素材の付与経路**を触る

該当しない通常の機能追加・バグ修正は 1 観点で足りる。

**なぜ**: 2026-07-26 にオーナーから「レビューの指摘が重箱の隅をつつきすぎ」
「これでは開発が進みません」と指摘。実測すると ★★★ はデータ破壊やクラッシュを
高い確率で捕まえていた一方、★ / ★★ が大量に出て**それを PR 内で全部拾っていたことが
遅さの主因**だった。危険な変更にだけ厚いレビューを当て、それ以外は薄くする。
...
|---|---|
| **★★★** | **必須対応** (マージ不可) |
| **★★** | **原則 issue 化して別 PR**。PR 本来のスコープ内で 1〜2 行で直るものだけ PR 内で直す |

**★★ を PR 内で拾いに行かない。** 拾うほど PR が肥大し、レビューをもう一周する羽目になる。
...
純 CSS・文言・docs **だけ** の小さな見た目調整は、イテレーションを速くするため
以下で軽量化してよい (ロジックが絡む変更には適用しない):

- **バッチ化**: 見た目の指摘を 1 個ずつ別 PR にせず、数個ためて 1 つの feature ブランチ
  / PR でまとめて直す (PR を量産してパイプラインを何周も回さない)。
- **レビュー省略可**: 値・色・余白・文言だけならレビューを飛ばしてよい
  (§1.5 の既定が 1 観点になったので、そこからさらに軽くする枠)。ただし
  **詳細度・上書き順・レイアウト計算・safe-area など論理が絡む変更は通常どおり 1 観点を回す**
  (過去、footer の詳細度バグ・透明 border 透けをレビューが実際に捕まえている)。
- **プレビュー待ち省略**: `web-ci` が緑になれば Cloudflare の **PR プレビュービルドを
  待たずにマージ**してよい (マージ後の dev ビルドで反映される)。
- **CI の e2e スキップ**: CI は CSS / docs だけの変更では Playwright/e2e smoke を自動
  スキップする (`.github/workflows/ci.yml` の `dorny/paths-filter`。typecheck / unit /
  build は常に回る)。`main` への push/PR は本番経路なので常に e2e を回す。

「ロジックが絡むか」の線引き: 値・色・余白・文言の単純変更は軽量パス。セレクタの
詳細度を変える / 構造 (DOM) を変える / JS 計算に触れる変更は通常パス (1 観点)。
...
- [ ] PR 作成後、**§1.5 のレビューを回す** (既定 1 観点。権威データ/データ消失/付与経路/本番リリースなら 3 観点)
...
これを全 PR の運用ルールに昇格させた (**2026-07-26 に既定 1 観点へ縮小**。§1.5 参照)。
マージ前に必ず subagent
